...
function InfluxdbBackend(startupTime, config, events) {
    ...
    self.defaultProxyAddSuffix = true;
    ...
    if (config.influxdb) {
        ...
        if (config.influxdb.proxy) {
            ...
            self.proxyAddSuffix = config.influxdb.proxy.addSuffix !== undefined ? config.influxdb.proxy.addSuffix : self.defaultProxyAddSuffix;
...
InfluxdbBackend.prototype.processFlush = function (timestamp, metrics) {
    ...
    for (key in rates) {
        /* Do not include statsd rates. */
        if (!self.includeStatsdMetrics && key.match(statsPrefixRegexp)) {
            continue;
        }

        var value = rates[key];
        var k = influxKey(key, 'rate');
    ...
    for (key in sets) {
        /* Do not include statsd sets. */
        if (!self.includeStatsdMetrics && key.match(statsPrefixRegexp)) {
            continue;
        }

        var k = influxKey(key, 'set');
        sets[k].map(function (v) {
            points.push(self.assembleEvent(k, [{value: v, time: timestamp}]));
        });
        points.push(self.assembleEvent(k + "_count", [{value: sets[k].length, time: timestamp}]));
    }
...
InfluxdbBackend.prototype.enqueue = function (type, ts, key, value) {
    ...
    key = key + '.' + type;
    if (self.proxyAddSuffix) {
        key = key + '.' + self.proxySuffix;
    }
